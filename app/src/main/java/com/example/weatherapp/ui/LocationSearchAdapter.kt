...
class LocationSearchAdapter(
    private var locations: List<SearchLocation>,
    private val onItemClick: (SearchLocation) -> Unit
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        "${location.name}, ${location.country}, ${location.region}".also { holder.locationName.text = it }
        holder.itemView.setOnClickListener { onItemClick(location) }
    }

    override fun getItemCount(): Int = locations.size

    fun updateList(newLocations: List<SearchLocation>) {
